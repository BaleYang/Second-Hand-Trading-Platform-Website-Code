...
function page($count,$page_size,$num_btn=10,$page='page'){
	...
	if($num_btn>=$page_num_all){
		...
		for($i=1;$i<=$page_num_all;$i++){//这边的$page_num_all是限制循环次数以控制显示按钮数目的变量,$i是记录页码号
			if($_GET[$page]==$i){
				$html[$i]="<div class='single_paginat active'>{$i}</div>";
			}else{
				$html[$i]="<a href='{$url}{$i}'><div class='single_paginat'>{$i}</div></a>";
			}
		}
	}else{
		...
		for($i=0;$i<$num_btn;$i++){
			if($_GET[$page]==$start){
				$html[$start]="<div class='single_paginat active'>{$i}</div>";
			}else{
				$html[$start]="<a href='{$url}{$i}'><div class='single_paginat'>{$i}</div></a>";
			}
			$start++;
		}
